...
export default function EditProfilePenghuni() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/auth/profile/setup`, {
        ...
        credentials: "include",
        // We only send nama and nomor_telepon for resident
        body: JSON.stringify(formData)
      });

      if (response.ok) {
        router.push("/penghuni/profile");
        router.refresh(); 
      } else {
    ...
    <main className="flex flex-col px-10 py-8 w-full max-w-4xl font-sans text-[#1a1a1a]">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        <div className="space-y-2">
          <label className="text-sm font-bold text-[#1a1a1a]">Nama</label>
          <input
            type="text"
            name="nama"
            value={formData.nama}
            onChange={handleChange}
            placeholder="Contoh: Zuhdi Arif Azizi"
