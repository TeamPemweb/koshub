...
export default function EditKamarPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 1. Fetch Room Types
      const resTypes = await fetch(`${apiUrl}/owner/room-types`, {
        credentials: "include"
      });
      if (resTypes.ok) {
      ...
      const resRooms = await fetch(`${apiUrl}/owner/rooms`, {
        credentials: "include"
      });
      if (resRooms.ok) {
        const rooms = await resRooms.json();
        // Cari kamar berdasarkan ID dari URL
        const currentRoom = rooms.find(r => r.ID === Number(roomId));
        if (currentRoom) {
          setNomorKamar(currentRoom.NomorKamar);
          setTipeKamarId(currentRoom.TipeKamarID);
        } else {
          console.error("Kamar tidak ditemukan");
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "";
      const response = await fetch(`${apiUrl}/owner/rooms/${roomId}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        console.error(data.message || "Terjadi kesalahan saat menyimpan perubahan kamar");
      }
    } catch (error) {
  ...
  return (
    <main className="flex flex-col px-10 py-8 w-full max-w-4xl font-sans text-[#1a1a1a]">
      ...
      {isFetchingData ? (
        <div className="text-gray-500">Memuat data kamar...</div>
      ) : (
          ...
          <div className="space-y-2 relative" ref={dropdownRef}>
            ...
            >
              <span className={selectedType ? "text-[#1a1a1a]" : "text-gray-400"}>
                {selectedType ? selectedType.NamaTipe : "Pilih tipe kamar"}
              </span>
            </div>
